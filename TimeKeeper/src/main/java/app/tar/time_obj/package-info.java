 ...
 * {@link app.tar.time_obj.Mode}{@code s}.
 * <p>The basic package class is abstract {@link app.tar.time_obj.Time_counter} class.
 ...
 * {@link app.tar.time_obj.Time_display_style}, {@link app.tar.time_obj.Time_unit_layout} and
 * displayed time&nbsp;units range setting. Each {@link app.tar.time_obj.Time_counter}
 * may have its own time layout settings. These settings may be changed
 * on&nbsp;the&nbsp;fly after time&nbsp;counter creation.
 * <p>For the&nbsp;case of {@link app.tar.time_obj.Time_unit_layout#TUL_full_name}
 * layout there is auxiliary
 * {@link app.tar.time_obj.Time_unit_full_name_resource_provider} class implemented (see
 ...
 * <p>{@link app.tar.time_obj.Solo_counter} and
 * {@link app.tar.time_obj.Instance_counter} extend {@link app.tar.time_obj.Time_counter} class.
 * <p>{@link app.tar.time_obj.Solo_counter} implements <i>stopwatch</i>
 * ({@link app.tar.time_obj.Mode#M_stopwatch}) and <i>countdown</i>
 * ({@link app.tar.time_obj.Mode#M_countdown}) modes. <i>Stopwatch</i> may be
 ...
 * <p>Time&nbsp;value may be edited after {@link app.tar.time_obj.Solo_counter} object
 ...
 * <p>{@link app.tar.time_obj.Instance_counter} implements so&nbsp;called
 * <i>elapsed&nbsp;from</i> ({@link app.tar.time_obj.Mode#M_elapsed_from}) and
 * <i>remains&nbsp;till</i> ({@link app.tar.time_obj.Mode#M_remains_till}) modes.
 ...
 * <p>The feature of both supported {@link app.tar.time_obj.Mode}{@code s}
 ...
 * come&nbsp;yet. In case of {@link app.tar.time_obj.Mode#M_elapsed_from}
 ...
 * {@link app.tar.time_obj.Mode#M_remains_till} has fully <u>opposite</u> behavior.
 * 
 ...
 * <p>{@link app.tar.time_obj.Solo_counter} and {@link app.tar.time_obj.Instance_counter} has
 * some differences.
 * <p>{@link app.tar.time_obj.Solo_counter} may provide counting in <u>two</u>&nbsp;ways.
 * They are represented by {@link app.tar.time_obj.Days_in_year} enumeration.
 * The&nbsp;key difference between them is only in days&#8209;in&#8209;month
 * quantity. {@link app.tar.time_obj.Days_in_year#DIY_360} has {@code 30}&nbsp;days in
 * each month, while {@link app.tar.time_obj.Days_in_year#DIY_365} has months quantity
 * to form {@code 365}&#8209;days year. Each {@link app.tar.time_obj.Solo_counter} is
 * set with {@link app.tar.time_obj.Days_in_year} value at creation time, and this value
 * <u>cannot</u> be changed later.
 * <p>{@link app.tar.time_obj.Instance_counter} counts time&nbsp;difference in
 ...
 * All {@link app.tar.time_obj.Time_counter} objects are controlled by
 * {@link app.tar.time_obj.Time_counter_control} class, which is implemented as
 * a&nbsp;singleton. Especially it concerns to {@link app.tar.time_obj.Instance_counter}
 * objects. {@link app.tar.time_obj.Time_counter_control} provides synchronous
 * per&#8209;second execution to them (because current&nbsp;time is mutual for
 * all!). But {@link app.tar.time_obj.Solo_counter} objects have individual
 * per&#8209;1/10th&nbsp;second task execution (however user&#8209;provided
 * accuracy is only {@code 1}&nbsp;second).
 * <p>Each {@link app.tar.time_obj.Time_counter} puts itself to
 * {@link app.tar.time_obj.containers.Modified_ArrayList} container at
 * the&nbsp;initialization time, which is stored in mentioned
 * {@link app.tar.time_obj.Time_counter_control} class.
 * <p>The last meaningful class that deserves attention is
 * {@link app.tar.time_obj.Settings}. It stores {@link app.tar.time_obj} package necessary
 * settings such as {@link app.tar.time_obj.Time_counter}'s values default layout and
